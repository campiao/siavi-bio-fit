...
def run_face_recognition(voice_thread, voice_thread_stop_flag):
    player = None
        ...
        if player and not register and found:
            ...
            if time.time() - ultimo_tempo > 7:
                ...
                if get_command() == "log out":
                    ...
                    close_window(cap, cv2,
                                 voice_thread,
                                 voice_thread_stop_flag)
                    break
...
def define_encoding(frame_rgb, ze, zd, zs, zi, player, name, gender, age, register) -> bool:
        ...
        if ze < centro_x < zd and zs < centro_y < zi:
